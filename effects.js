(function() {
    ...
    if (emojiToggle) {
        // Clear potential old interval if this script runs multiple times
        if (window.currentRainInterval) clearInterval(window.currentRainInterval);

        emojiToggle.addEventListener("change", () => {
            if (emojiToggle.checked) {
                if (window.currentRainInterval) clearInterval(window.currentRainInterval);
                rainInterval = setInterval(createPixelRain, 100);
                window.currentRainInterval = rainInterval;
            } else {
                clearInterval(rainInterval);
                window.currentRainInterval = null;
