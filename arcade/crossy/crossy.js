class CrossyGame {
    ...
    draw() {
        ...
        const bgColor = isRetro ? '#0d1117' : '#0d1117'; // Keep dark
        this.ctx.fillStyle = bgColor;
        this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);
        
        const viewOffset = 400; // Player visually at 400px down

        this.lanes.forEach(lane => {
            // Screen Y = Lane.y (World) - CameraY + viewOffset
            const drawY = lane.y - this.cameraY + viewOffset;
            
            // Optimization: cull offscreen
            if (drawY < -50 || drawY > this.canvas.height + 50) return;
            
            // Draw Lane
            if (lane.type === 'grass') {
                this.ctx.fillStyle = isRetro ? '#238636' : '#238636';
                if (isRetro) { // Stripes
                     // this.ctx.fillRect(0, drawY, this.canvas.width, this.gridSize);
                     // Checkered?
                     for(let c=0; c<this.columns; c++) {
                         this.ctx.fillStyle = (c + lane.index)%2 === 0 ? '#2ea043' : '#238636';
                         this.ctx.fillRect(c*this.gridSize, drawY, this.gridSize, this.gridSize);
                     }
                } else {
                     ...
                     this.ctx.fillRect(0, drawY, this.canvas.width, 5);
                }
                ...
                this.ctx.fillRect(0, drawY + this.gridSize - 5, this.canvas.width, 2);
            ...
            lane.elements.forEach(el => {
                ...
                if (!isRetro) {
                    // Slight 3D effect
                    this.ctx.fillRect(el.x, drawY + 10, el.width, el.height);
                    this.ctx.fillStyle = '#00000033'; // Shadow side
                    this.ctx.fillRect(el.x, drawY + 10 + el.height, el.width, 5);
                } else {
                    // Flat
                    this.ctx.fillRect(el.x, drawY + 10, el.width, el.height);
                }
        ...
        if (this.player && !this.player.isDead) {
             const drawY = (this.player.gridY * this.gridSize) - this.cameraY + viewOffset;
             const drawX = this.player.x + (this.gridSize - this.player.width) / 2;
             
             // Shadow
             this.ctx.fillStyle = 'rgba(0,0,0,0.5)';
             this.ctx.fillRect(drawX + 5, drawY + this.gridSize/2 + 10, this.player.width, 10);

             // Body
             this.ctx.fillStyle = isRetro ? '#f0f6fc' : '#ffffff';
             // Scale for hop (Y scale)
             const h = this.player.height * this.player.squish;
             const w = this.player.width * (1 + (1 - this.player.squish)); // Bulge width if squished
             
             this.ctx.fillRect(drawX, drawY + (this.gridSize - h)/2, w, h);
             
             // Eyes
             this.ctx.fillStyle = '#000';
             this.ctx.fillRect(drawX + w - 10, drawY + (this.gridSize - h)/2 + 5, 5, 5);
             
             // Beak
             this.ctx.fillStyle = '#ffdf5d';
             this.ctx.fillRect(drawX + w, drawY + (this.gridSize - h)/2 + 10, 5, 5);
        }
    }
