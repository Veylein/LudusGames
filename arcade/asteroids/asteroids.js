class AsteroidsGame {
    ...
    init() {
        ...
        if(this.btnLeft) {
            this.btnLeft.addEventListener('touchstart', (e) => { e.preventDefault(); this.ship.rot = 0.1; }); // Left adds angle? No, usually sub
        ...
        if(this.btnFire) {
            this.btnFire.addEventListener('click', () => this.shoot());
        }
    ...
    newLevel() {
        this.level++;
        this.asteroids = [];
        const num = 2 + this.level; // Increase asteroids
        for(let i=0; i<num; i++) {
            let x, y;
            do {
                x = Math.floor(Math.random() * this.canvas.width);
                y = Math.floor(Math.random() * this.canvas.height);
            } while(this.dist(x, y, this.ship.x, this.ship.y) < 100); // Keep away from ship
            
            this.asteroids.push(this.newAsteroid(x, y, Math.ceil(Math.random() * 30) + 30)); // 30-60 radius
        }
    }
    ...
    shoot() {
        ...
        this.bullets.push({
            ...
            vx: this.ship.vx + 5 * Math.cos(this.ship.a), // Bullet speed 5
            vy: this.ship.vy + 5 * Math.sin(this.ship.a),
            ttl: 60 // Time to live (frames)
        });
    }
