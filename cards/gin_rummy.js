...
function updateGinUI() {
	...
	// Deck
	ginDeckDiv.innerHTML = `<div class='card back'>Deck (${ginDeck.length})</div>`;
	ginDeckDiv.onclick = () => ginRummyAction('draw');
	// Discard
	ginDiscardDiv.innerHTML = ginDiscard.length ? `<div class='card show'>${ginDiscard[ginDiscard.length-1]}</div>` : "";
